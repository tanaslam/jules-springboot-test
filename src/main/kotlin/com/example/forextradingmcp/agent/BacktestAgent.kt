...
class BacktestAgent(
    ...
    fun runBacktest(
        ...
        for (pair in context.currencyPairs) {
            logger.info("Running backtest for pair: {}", pair)
            try {
                ...
                if (barSeries.isEmpty) {
                    logger.warn("No historical data fetched for pair {}, skipping backtest for this pair.", pair)
                    continue
                }
        ...
        if (allSignals.isEmpty() && context.currencyPairs.isNotEmpty()) {
            ...
            // logger.info("Overall estimated win rate: {}%", winRate * 100)
            ...
            logger.info("Consolidated signals exported to {} directory, filename {}", outputDirectory, consolidatedFileName)

        } else {
             logger.info("Backtest completed. No signals generated and no pairs were specified in the context.")
        }
